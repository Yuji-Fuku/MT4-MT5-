...
int GetBarShift_M5(datetime t)
{
    ...
    int cnt = CopyTime(NULL, PERIOD_CURRENT, 0, 500, arr);
    if(cnt <= 0) return -1;
    for(int i = 0; i < cnt; i++)
        if(arr[i] <= t) return i;
    return -1;
...
int OnInit()
{
    ...
    h_stoch  = iStochastic(NULL, PERIOD_CURRENT, Stoch_K, Stoch_Smooth, Stoch_D, MODE_SMA, STO_CLOSECLOSE);
    ...
    for(int i = 0; i < 3; i++)
    {
        ...
        h_htf_stoch[i]  = iStochastic(NULL, htf_tfs[i], Stoch_K, Stoch_Smooth, Stoch_D, MODE_SMA, STO_CLOSECLOSE);
